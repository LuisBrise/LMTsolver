...
sprintf(filenamex, "Results/LyvAl/%d/dldw_a%.2gnm_v%.2g_b%.2gnm.dat", Lmax, a/(1.*nm), vv, b/(1.*nm));
...
for (Lmax = 1; Lmax < 16; Lmax++){
cout << "Lmax = " << Lmax << endl;
cout << endl;
char filename[sizeof "Results/DL_a1nm_v0.99c_b1.5nm_extF2.dat"];
sprintf(filename, "Results/LyvAl/%d/DL_a%.2gnm_b%.2gnm.dat", Lmax,a/(1.*nm), b/(1.*nm));
...
sprintf(filenamer, "Results/LyvAl/%d/DL_a%.2gnm_b%.2gnm_error.dat", Lmax, a/(1.*nm), b/(1.*nm));
