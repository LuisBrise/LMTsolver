...
*/  
//	flags 
//      g++ -o OUT/DLyGK_exact_vs_v.out DLyGK_exact_vs_v.cpp -lcomplex_bessel -w 
//      g++ -o OUT/DLyGK_exact_vs_v.out DLyGK_exact_vs_v.cpp -lcomplex_bessel -w && ./OUT/DLyGK_exact_vs_v.out 
//
...
//typedef boost::multiprecision::cpp_bin_float_50  dfloat;          // Much higher precision than quad
...
sprintf(filenamex, "Results/%s/%d/dldw_a%.2gnm_v%.2g_b%.2gnm.dat", directoryLv, Lmax, a/(1.*nm), vv, b/(1.*nm));
              ...
              for (int m2 = -l2; m2 <= l2; m2++){
                   dm2 = 1.*m2;     
                    ...
                    if(std::abs (m2-m1)==1){
                                    ...
                                    // Radial - Radial 
                        normfactor = pow((2.*dl1+1.)*(dl1+dm1+1.)/((2.*dl1+3.)*(dl1-dm1+1.)),0.5);
...
cout << endl;
//****************************************************************************************************
cout << R"(DLGK_exact_vs_v.cpp

Description:
This program calculates the spectral linear momentum (dL/dω) transferred to a 
nanoparticle (NP) due to the interaction with a fast-moving electron. The calculation 
is performed by analytically solving the closed surface integral, resulting in a 
...
cout << "Select an option to input parameters:" << endl;
    ...
    cout << R"(4. Select the following parameters
                ...
                double vvInit = 0.5;
                double vvFin = 0.95)" << endl;
    cout << R"(5. Load parameters from parameters_vs_v.csv file
                Note that the file should be written in the form:
                    a,b,r,vvInit,vvFin
                    50.0,51.5,50.05,0.5,0.95)" << endl;
    cout << "Option:  ";
    cin >> choice;
cout << endl;

double a, b, r, vvInit, vvFin, vv;
...
if (choice == 1) {                        
        ...
        string filename = "Parameters/parameters_vs_v.csv";
        cout << R"(Reading the parameters in the parameters_vs_b.csv filename. 
                    Note that the file should be written in the form:
                    a,b,r,vvInit,vvFin
                    50.0,51.5,50.05,0.5,0.95)";
        //cin >> filename;
        ...
        getline(ss, value, ',');
        cout << "vvInit = " << stod(value) << "*c \n";
        vvInit = stod(value);

        getline(ss, value, ',');
        cout << "vvFin = " << stod(value) << "*c \n";
        vvFin = stod(value);
...
dcomplex DLy[6];
const double errorThreshold = 0.0001; // 1% threshold
...
sprintf(filenamer, "Results/%s/%d/DL_a%.2gnm_b%.2gnm_error.dat", directoryLv, Lmax, a/(1.*nm), b/(1.*nm));
...
{
   vv = (vvInit + (vvFin-vvInit)*i/numOfPoints);
   DL(r, vv, b, a, DLy);
   double Ly=0.0;
   ...
   fprintf(fppe,"%.17g %.17g %.17g %.17g %.17g %.17g %.17g \n",vv,DLy[0].imag(),DLy[1].imag(),DLy[2].imag(),DLy[3].imag(),DLy[4].imag(),DLy[5].imag());
