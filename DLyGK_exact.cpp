...
*/  
//	flags 
//      g++ -o OUT/DLyGK_exact.out DLyGK_exact.cpp -lcomplex_bessel -w 
//      g++ -o OUT/DLyGK_exact.out DLyGK_exact.cpp -lcomplex_bessel -w && ./OUT/DLyGK_exact.out 
//
...
char filenamex[sizeof "Results/50/Convergence-Analysis_dldw_a1nm_v0.99c_b1.5nm_extF2.dat"];
sprintf(filenamex, "Results/%d/Convergence-Analysis_dldw_a%.2gnm_v%.2g_b%.2gnm.dat", Lmax, a/(1.*nm), vv, b/(1.*nm));
...
- The program supports multiple parameter sets and stops when the solution converges 
  to a specified relative error threshold or reaches the maximum multipolar order (Lmax = 50).
    ...
    cout << R"(Load parameters from Parameters/parameters.csv file
                Note that the file should be written in the form:
                    a,b,r,vv
                    50.0,51.5,50.05,0.95)" << endl;
cout << endl;

double a, b, r, vv;

        string filename = "Parameters/parameters.csv";
        cout << R"(Reading the parameters in the parameters.csv filename. 
                    Note that the file should be written in the form:
                    a,b,r,vv
                    50.0,51.5,50.05,0.95)";
        //cin >> filename;
...
cout << "Select value of multipolar order L:  ";
    cin >> Lmax;
cout << endl;
