...
//      g++ -o DLyGK_exact_vs_v_Drude.out DLyGK_exact_vs_v_Drude.cpp -lcomplex_bessel -w && ./DLyGK_exact_vs_v_Drude.out 
//
...
double w3 = .3;
double w4 = 2.;
double w5 = 50.; // Omega cut for freq. integral
...
sprintf(filenamex, "Results/LyvDrude/%d/dldw_a%.2gnm_v%.2g_b%.2gnm.dat", Lmax, a/(1.*nm), vv, b/(1.*nm));
...
for (Lmax = 21; Lmax < 31; Lmax++){
cout << "Lmax = " << Lmax << endl;
cout << endl;
char filename[sizeof "Results/DL_a1nm_v0.99c_b1.5nm_extF2.dat"];
sprintf(filename, "Results/LyvDrude/%d/DL_a%.2gnm_b%.2gnm.dat", Lmax,a/(1.*nm), b/(1.*nm));
...
sprintf(filenamer, "Results/LyvDrude/%d/DL_a%.2gnm_b%.2gnm_error.dat", Lmax, a/(1.*nm), b/(1.*nm));
