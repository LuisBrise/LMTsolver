...
return 1. + LL(w, 0.                , 0.007 , 0.153)
          + LL(w, 0.147 , 0.055 , 0.060);
} 
...
sprintf(filenamex, "Results/LybLorentz/%d/dldw_a%.2gnm_v%.2g_b%.2gnm.dat", Lmax, a/(1.*nm), vv, b/(1.*nm));
...
sprintf(filename, "Results/LybLorentz/%d/DL_a%.2gnm_v%.2g.dat", Lmax,a/(1.*nm), vv);
...
sprintf(filenamer, "Results/LybLorentz/%d/DL_a%.2gnm_v%.2g_error.dat", Lmax, a/(1.*nm), vv);
